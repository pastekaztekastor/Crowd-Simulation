...
__global__ void kernel_model1_GPU(
    ...
    if(tid < simDimP){
        ...
        int deltaX =  simExit[0]-x / max(simExit[0]-x, simExit[1]-y);
        int deltaY =  simExit[1]-y / max(simExit[0]-x, simExit[1]-y);
        // on regarde si la case est disponible 
        if((*dev_map[y+deltaY][x+deltaX]) == 0){ // if is EMPTY
            (*dev_populationPosition[tid][0]) = x+deltaX;
            (*dev_populationPosition[tid][1]) = y+deltaY;
