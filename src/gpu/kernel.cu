...
void initKernelParam(kernelParam * _kernelParam, simParam _simParam, settings _settings){
    ...
    if( _settings.print > 2 )cout  << " \t> dev_ variable";
    ...
    if( _settings.print > 2 )cout << " \t> Maloc";
    ...
    if(tid < _simParam.nbIndividual)
    {
        if ( (*_kernelParam.populationPosition)[tid].x > -1 && (*_kernelParam.populationPosition)[tid].y > -1){
            ...
            if(atomicExch(_kernelParam.map[ _simParam.dimension.x * (pos.y + move.y) + (pos.x + move.x)], tid) == tid){
                if((*_kernelParam.map)[ _simParam.dimension.x * (pos.y + move.y) + (pos.x + move.x)] == __MAP_EMPTY__){         // __MAP_EMPTY__
                ...
                else if((*_kernelParam.map)[ _simParam.dimension.x * (pos.y + move.y) + (pos.x + move.x)] == __MAP_EXIT__){     // __MAP_EXIT__
...
void mapKernelToSim(kernelParam _kernelParam, simParam * _simParam, settings _settings){
    if( _settings.print > 2 )cout <<endl<< " \t> mapKernelToSim ";
...
void popKernelToSim(kernelParam _kernelParam, simParam * _simParam, settings _settings){
    if( _settings.print > 2 )cout <<endl<< " \t> popKernelToSim ";
